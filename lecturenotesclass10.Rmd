...
```{r git-config}
library(usethis)
use_git_config(user.name= "roryqsmith", user.email="rory.smith@duke.edu")
```
...
The marginal probability is 43/84 or approximately 52%.

- What is the probability a randomly selected patient received the placebo
  and had a marked improvement?
  
The probability is 7/84 or 8.3%.
  
- What is the probability a randomly selected patient received the placebo
  and the treatment?
  
The probability is zero - this is a disjoint probability.
  
- What is the probability a randomly selected patient had some improvement or
  was on the treatment?
  
The probability is 48/84 or 57%.
...
```{r simulate_draws}
draw_results <- sample(marbles, size = 100000, replace = TRUE)
```
...
All of the values are close to the true probabilities. The first time I ran the simulation, the white marble's probability is the same, and the blue and red are just slightly off. This reflects the Law of Large Numbers.
...
I think that the probability will be larger than expected, since there are 3 and 4 instances of the same color. However, it isn't very likely since neither probability has a majority in the entire sample space.
...
Two marbles of the same draw are picked 39.5% of the time.
...
```{r simulate_rolls}
roll_results <- tibble(
  roll_1 = sample(1:6, size = 10000, replace = TRUE),
  roll_2 = sample(1:6, size = 10000, replace = TRUE)
)
roll_results
```
...
roll_results <- roll_results %>% 
  mutate(square_rolls = roll_1 ^ 2 == roll_2) %>%
  mutate(abs_rolls = abs(roll_1 - roll_2) == 4)
roll_results
```

```{r summarize_rolls}
roll_results %>% 
  summarize(roll1_prob = mean(roll_1),
            roll2_prob = mean(roll_2))
```
